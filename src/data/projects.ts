...
export const projects: ArcUnitProject[] = [
  ...
  },
  {
    ...
    deepDiveTitle: 'Detect cycles across a Rails application.',
    deepDiveCopy:
      'A cycle rule checks the selected application files as one directed graph and reports the complete paths that close a loop. Use a layer or slice rule when the decision is about independence between named business domains.',
    deepDiveCode: [
      "require 'archunit'",
      '',
  ...
  },
  {
    ...
    description:
      'Public-beta architecture testing for compiled Java code, with deterministic policies, JUnit integration, and CI-ready evidence.',
    longDescription:
      'Read class directories and JARs without loading target classes, build an immutable dependency model, and evaluate architecture policies with explicit completeness. ArchUnitJava 0.1.0 is available from Maven Central for JUnit, CLI, and lower-level Java API workflows.',
    languageOverview:
      'Java is a statically typed language that runs on the Java Virtual Machine and anchors a large ecosystem of enterprise, Android, data, and backend software. Maven and Gradle organize builds, while Spring and Jakarta EE are common application platforms. ArchUnitJava analyzes compiled bytecode as data, so target classes are never loaded or initialized during the check.',
    ecosystemHighlights: [
      'JVM portability and a mature enterprise ecosystem',
      'Spring, Jakarta EE, Maven, Gradle, and JUnit 5',
      'Compiled class and JAR analysis without application startup',
    ],
    status: 'preview',
    statusLabel: 'Public beta',
    accent: '#ff9d75',
    accentSoft: 'rgba(255, 157, 117, 0.14)',
    install: 'testImplementation("io.github.tristankruse:archunitjava:0.1.0")',
    installLabel: 'Gradle / Maven Central',
    code: [
    ...
    deepDiveTitle: 'Run architecture policy with ordinary JUnit.',
    deepDiveCopy:
      'The recommended public-beta path keeps policy in a strictly validated properties file and invokes the CLI bridge from a normal JUnit test. Compile first, then let the same test command used by the project enforce type or package dependency rules.',
    deepDiveCode: [
      '@Test',
      'void compiledApplicationRespectsItsPolicy() {',
      '    Path root = Path.of("").toAbsolutePath().normalize();',
      '    StringBuilder output = new StringBuilder();',
      '    StringBuilder error = new StringBuilder();',
      '',
      '    int exit = new CliRunner().run(new String[] {',
      '        "check", "--config", "archunitjava.properties",',
      '        "--root", root.toString(), "--result-format", "json"',
      '    }, output, error);',
      '',
      '    assertEquals(CliExitCode.SUCCESS.code(), exit, error.toString());',
      '}',
    ].join('\n'),
    deepDiveFilename: 'ArchitectureTest.java',
    docsUrl: 'https://lukasniessen.github.io/ArchUnitJava/',
    packageUrl: 'https://central.sonatype.com/artifact/io.github.tristankruse/archunitjava/0.1.0',
    packageLabel: 'View 0.1.0 on Maven Central',
    features: [
      'Class-directory, ordinary JAR, and multi-release JAR imports',
      'Type, package, member, module, layer, and slice policies',
      'Cycles, reachability, PlantUML, metrics, and baselines',
      'Ordinary JUnit tests and optional JUnit Platform discovery',
      'Console, JSON, SARIF, JUnit XML, and graph reports',
      'Explicit incomplete-analysis and empty-selection behavior',
    ],
    useCases: ['Spring services', 'Maven monorepos', 'Gradle builds', 'JPMS applications'],
    maturityNote:
      'Version 0.1.0 is a published public beta suitable for evaluation and new architecture-test suites. Its pre-1.0 Java API may change between minor versions, and the established third-party ArchUnit library remains the mature default for production Java systems.',
    release: '0.1.0 public beta',
    runtime: 'JDK 25; analyzes Java 8 to 25 bytecode',
    license: 'Apache-2.0 (0.1.0)',
  },
  {
    slug: 'php',
    name: 'ArchUnitPHP',
